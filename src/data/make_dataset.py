...
def main(input_filepath, output_filepath):
    ...
    train, test = train_test_split(raw, test_size=0.3, random_state=1)
    ...
    total_payed = (raw.PAY_AMT1 + raw.PAY_AMT2 + raw.PAY_AMT3 + raw.PAY_AMT4 + raw.PAY_AMT5 + raw.PAY_AMT6)
    total_due = (raw.BILL_AMT2 + raw.BILL_AMT3 + raw.BILL_AMT4 + raw.BILL_AMT5 + raw.BILL_AMT6)
    raw['RELATIVE_DEBT'] = (total_payed - total_due) / raw['LIMIT_BAL']
    ...
    V = sp.std(raw['RELATIVE_DEBT'])
    raw['RELATIVE_DEBT'] = (raw['RELATIVE_DEBT'] - raw['RELATIVE_DEBT'].mean()) / V
    ...
    train, test = train_test_split(raw, test_size=0.3, random_state=1)
    train.to_csv(project_dir + '/data/processed/train.csv', index=False)
    test.to_csv(project_dir + '/data/processed/test.csv', index=False)
